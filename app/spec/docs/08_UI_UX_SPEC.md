...
데스크톱에서는 입력과 상담 결과를 화면 가로 중앙에 고정한다. 현재 제품 후보와 누적 환자 정보는 중앙 영역의 위치를 바꾸지 않는 독립된 오른쪽 보조 열에 표시하고, 보조 열의 상단은 입력 카드 상단과 맞춘다. 오른쪽 공간이 부족한 화면에서는 보조 열을 중앙 영역 아래로 이동한다.

입력창에 새로 들어오는 문장은 항상 손님의 발화다. 결과 영역의 문장은 약사나 약국 상담자가 손님에게 말할 제안이다. AI와 로컬 엔진은 두 역할을 별도 필드로 보존하며, 손님이 말하지 않은 증상·신체 부위·기간을 상담 기록에 추가하지 않는다. 화면은 최근 손님 발화와 상담자 문구를 서로 다른 제목으로 표시한다.
...
- `/`로 포커스
- 입력값은 손님 발화로 저장하며 상담자 문구로 재해석하지 않음
- IME composition 지원
...
| `Esc` | 열린 modal/cancel만 닫기. 상담 전체와 누적 환자 정보는 유지 |
| `1` `2` `3` | 후보 intent 선택 |
| `Enter` | 다음 질문/선택 확정 |
| `F` | freeze/unfreeze |
| `R` | 보정 요청 |
