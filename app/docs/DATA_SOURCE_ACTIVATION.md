...
3. 모든 유효성분을 정확한 기존 별칭 또는 공식 성분 문자열의 안정적인 해시 ID로 식별한다. 복합제는 개별 성분 안전 검사를 유지하면서 전체 조성을 별도 치료 단위로 연결한다.
4. 제품 단위 공식 효능·효과, 제형, 투여경로가 증상 경로와 일치한다. 복합제의 모든 성분이 하나의 기존 option을 공유해야 한다는 교집합 조건은 사용하지 않는다.
...
약국 실습자료의 증상별 구분, 제형 차이, 주 치료·보조 치료 원칙은 `data/clinical-pathways/pathways.json`에 구조화한다. 제품 분류는 다음 명령으로 다시 생성한다.

```powershell
pnpm data:build:clinical-pathways
node scripts/build-actual-preview-pack.mjs
```

현재 공식 확인 SKU 458개는 직접 증상 경로 266개와 보조 분류 192개로 빠짐없이 나뉜다. 직접 증상 경로 중 허가 취소·소스 충돌이 없는 254개 SKU만 연구 미리보기 팩에 들어간다. 보조 분류 제품은 상담 경로와 약사 검토가 추가되기 전까지 자동 추천하지 않는다.

추천 엔진은 기존 약사 검토 option을 우선 유지하고 새 공식 제품은 대안 후보로 정렬한다. 병용 후보는 같은 증상 경로 안에서 다음 조건을 모두 만족할 때만 만든다.

- 주 치료 제품과 보조 제품의 역할이 명시되어 있다.
- 주 치료 경로가 보조 기전을 호환 대상으로 선언했다.
- 두 제품의 유효성분이 겹치지 않는다.
- 두 제품이 각각 현재 환자 정보와 안전 규칙을 통과했다.

병용 후보도 `pharmacist_approved=false`인 연구 미리보기이며 운영 추천으로 승격하지 않는다. 동물약 실습자료의 종·체중·연령·임신 조건은 사람용 OTC 경로에 합치지 않는다.
