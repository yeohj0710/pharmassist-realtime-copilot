    ...
    const response = await this.client.responses.create(
      {
        ...
        input: [
          ...
          {
            role: "developer",
            // Key order is load-bearing, not cosmetic: prompt caching only
            // reuses an exact prefix, so the catalog has to serialize ahead of
            // anything that changes per turn. With previous_intent first, the
            // prefix diverged before the catalog and it was billed in full on
            // every call. Keep the per-turn fields last.
...
export const PHARMACY_COUNSELOR_COMPOSITION_PROMPT =
  ...
  "Say one piece of general self-care advice at most once in a consultation, never again. Sending the customer to a doctor or pharmacist is not general advice; referral_required alone controls that. " +
  ...
  "When referral_required is true, name no product and tell the customer plainly that this should be looked at by a pharmacist or a doctor first. When referral_required is false, never send the customer to a doctor, hospital, pharmacist, pharmacy, or emergency service; ask or recommend within verified_products instead. " +
...
export interface CounselorCompositionContext {
  ...
  /** What tells those products apart, so one can be preferred over another. */
  readonly productGuidance?: readonly Readonly<{
    name: string;
    chooseWhen: string;
    differentiators?: readonly string[];
  }>[];
  /** Pairs the engine judged safe to take together, and why each is there. */
  readonly combinationGuidance?: readonly Readonly<{
    primary: string;
    supportive: string;
    rationale: string;
  }>[];
        ...
        input: [
          { role: "system", content: PHARMACY_COUNSELOR_COMPOSITION_PROMPT },
          {
            ...
            content: JSON.stringify({
              ...
              product_guidance: (context.productGuidance ?? []).map((item) => ({
                name: item.name,
                choose_when: item.chooseWhen,
                differentiators: item.differentiators ?? [],
              })),
              combination_guidance: context.combinationGuidance ?? [],
